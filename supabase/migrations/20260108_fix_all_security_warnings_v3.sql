-- ============================================
-- COMPREHENSIVE SECURITY FIX (Version 3 - Final Fix)
...
-- ============================================
-- PART 1: RLS Helper Functions (in public schema)
-- ============================================
...
RETURNS TEXT AS $$
  SELECT name FROM users WHERE id = public.get_user_id();
$$ LANGUAGE sql STABLE SECURITY DEFINER;
...
RETURNS BOOLEAN AS $$
  SELECT COALESCE(
    (SELECT name = 'Derrick' FROM users WHERE id = public.get_user_id()),
    false
  );
$$ LANGUAGE sql STABLE SECURITY DEFINER;
...
COMMENT ON FUNCTION public.rls_enabled() IS 'Feature flag for gradual RLS rollout. Returns false by default for backward compatibility.';
...
CREATE POLICY "rls_users_select"
  ...
  USING (
    CASE
      WHEN public.rls_enabled() THEN true  -- All users can see user list
      ELSE true
    END
  );
...
CREATE POLICY "rls_users_update"
  ...
  USING (
    CASE
      WHEN public.rls_enabled() THEN (id = public.get_user_id() OR public.is_admin())
      ELSE true
    END
  );
...
CREATE POLICY "rls_users_delete"
  ...
  USING (
    CASE
      WHEN public.rls_enabled() THEN public.is_admin()
      ELSE true
    END
  );
...
CREATE POLICY "rls_todos_insert"
  ...
  WITH CHECK (
    CASE
      WHEN public.rls_enabled() THEN created_by = public.get_user_name()
      ELSE true
    END
  );
...
CREATE POLICY "rls_messages_select"
  ...
  USING (
    CASE
      WHEN public.rls_enabled() THEN (
        created_by = public.get_user_name() OR
        recipient = public.get_user_name() OR
...
CREATE POLICY "rls_activity_insert"
  ...
  WITH CHECK (
    CASE
      WHEN public.rls_enabled() THEN user_name = public.get_user_name()
      ELSE true
    END
  );
...
CREATE POLICY "rls_templates_select"
  ...
  USING (
    CASE
      WHEN public.rls_enabled() THEN (
        is_shared = true OR
        created_by = public.get_user_name()
      )
      ELSE true
    END
  );
  ...
  IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'leads') THEN
    EXECUTE '
      CREATE POLICY "rls_leads_select"
        ...
        USING (
          CASE
            WHEN public.rls_enabled() THEN true
            ELSE true
          END
        )
    ';
...
-- Skip if functions don't exist - they may not be in use
-- ============================================

-- Fix append_attachment_if_under_limit function (with specific signature)
DO $$
BEGIN
  IF EXISTS (
    ...
    AND p.proname = 'append_attachment_if_under_limit'
  ) THEN
...
BEGIN
  IF EXISTS (
    ...
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public'
    AND p.proname = 'cleanup_old_device_tokens'
  ) THEN
    -- Re-create function with proper security settings
...
BEGIN
  ...
  RAISE NOTICE '🛠️ Function security issues fixed (if functions exist)';
  RAISE NOTICE '';
